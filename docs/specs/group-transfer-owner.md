...
- 処理
  ...
  - 上記を満たせば、`ownerId` のみ `newOwnerId` に更新した新しい `Group` を返す。`name` と `members` は変更しない（旧オーナーも `members` に残ったままにする。`members` からの削除は `removeMember` を別途呼ぶかどうかの呼び出し側の判断とする）
- 出力
...
- [ ] 委譲後、旧オーナーは `members` に残ったままである（`members` からは自動的に外れない）
- [ ] 委譲後、`name` と `members` は変更前と同じ値のまま保たれる
- [ ] 変更前の `group` オブジェクト自体は変更されない（`transferOwner` はイミュータブルに新しい `Group` を返す）

## 対象外

- 委譲後に旧オーナーを `members` から自動的に外すこと（呼び出し側が必要なら `removeMember` を別途呼ぶ想定。本仕様では扱わない）
- 委譲先が現在のオーナー自身（`newOwnerId === group.ownerId`）の場合の挙動（no-op として成功させるか、エラーにするかは仕様として未定義。人への質問として別途報告する）
